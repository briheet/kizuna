...
func TestSearch(t *testing.T) {
	...
	app := api.NewApi(t.Context(), &config.Config{
		Embedder: config.EmbedderConfig{BaseURL: nomic.URL},
	...
	err := db.Client.ExecuteTx(t.Context(), func(tx pgx.Tx) error {
		...
		_, err := tx.Exec(t.Context(), `insert into chunks (id, graph_node_id, chunk_index, content, embedding, created_at) values ($1, $2, 0, 'nearest chunk', $3::VECTOR, now()), ($4, $5, 0, 'far chunk', $6::VECTOR, now());`, uuid.New(), nodeA, vector(1), uuid.New(), nodeB, vector(-1))
		return err
	})
	...
	require.Len(t, resp.RelatedNodes, 2)
